    ...
    <Container className="mt-3">
      ...
      <Row className="d-flex">
        ...
        <Col xs={12} sm={6} className="d-flex justify-content-center">
          <Card className="d-page__card d-flex flex-column align-items-center justify-content-around px-3 py-3">
            <h3>Стоимость</h3>
            <h3>{deviceLocal?.price} ₽</h3>
            {user.isAuth && user.user?.role === 'ADMIN' && (
              <>
                <Button
                  variant={`${theme === 'light' ? 'outline-dark' : 'outline-light'} button-admin mb-2`}
            ...
            <Button variant={`${theme === 'light' ? 'outline-dark' : 'outline-light'} button-admin mb-2`} onClick={add}>
              Добавить в корзину
            </Button>
          </Card>
        </Col>
        ...
        {deviceLocal?.info.map((info, index) => (
          <Row
            ...
            style={{
              ...
              padding: 10,
              color: index % 2 === 0 ? 'black' : 'lightgray',
            }}
            className="mx-0"
          >
      ...
      <DeviceModal
        ...
        titleModal="Редактировать"
        currentDevice={deviceLocal}
      />
    </Container>
  )
}
