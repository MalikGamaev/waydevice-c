...
const DeviceModal: FC<CreateProps> = observer(({ show, onHide, titleModal, currentDevice }) => {
  ...
  const uploadImage = async (file: File) => {
    ...
    const response = await fetch(`https://api.cloudinary.com/v1_1/dxzzglv48/image/upload`, {
      method: 'POST',
      body: formData,
    })
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">{titleModal} устройство</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          <Dropdown className="mt-2 mb-2">
            <Dropdown.Toggle variant={theme === 'light' ? 'primary' : 'dark'}>
          ...
          <hr />
          <Button variant={theme === 'light' ? 'outline-dark' : 'outline-light'} onClick={addInfo}>
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-success" onClick={deviceClick}>
          {titleModal}
        </Button>
      </Modal.Footer>
    </Modal>
